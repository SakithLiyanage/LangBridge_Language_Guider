  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 mb-10 flex flex-col gap-6">
          <div className="flex flex-col md:flex-row md:items-center gap-6">
            ...
            <div className="flex flex-col gap-2 mt-4 md:mt-0">
              <button className="btn-primary w-full md:w-auto" onClick={handleDocTranslate} disabled={!docFile || docLoading}>
                ...
                {docLoading ? 'Processing...' : 'Translate & Summarize'}
              </button>
            </div>
          </div>
          {docResult && (
            <div className="grid md:grid-cols-3 gap-6 mt-4">
              ...
              <div className="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 shadow flex flex-col">
                <h4 className="font-semibold mb-2 flex items-center gap-2 text-green-700 dark:text-green-300"><Award className="w-4 h-4" /> AI Summary</h4>
                <div className={`text-sm ${docResult.summary?.startsWith('AI summarization failed') || docResult.summary?.startsWith('Not enough text') ? 'text-red-600' : 'text-green-900 dark:text-green-100'}`}>{docResult.summary}</div>
              </div>
            </div>
          )}
        </div>
